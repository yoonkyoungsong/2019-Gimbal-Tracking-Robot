...
#include "header.h"
	...
	//Vcmd[i] = LinearCheckWave(i, Vcmd[i - 1]);
...
double TriangulerWave(double magnitude, double  TRI_Frequency, double CurrentTime_tri) {
	...
	if (CurrentTime_tri > Tri_time * (ct + 1))
			...
			{
				Tri_out = -magnitude / (Tri_time * 0.25) * (CurrentTime_tri - Tri_time * 0.25 - Tri_time * ct) + 2.5;
			...
			{
				Tri_out = magnitude / (Tri_time * 0.25) * (CurrentTime_tri - Tri_time * 0.75 - Tri_time * ct) - 2.5;
